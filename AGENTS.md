...
<todo_pause_conditions>
## 暫停等待用戶確認的情況

系統可能自動發送 `[SYSTEM REMINDER - TODO CONTINUATION]` 指示繼續執行未完成的 todo。在大多數情況下應遵循，但遇到以下情況時，必須**停止並等待用戶確認**，而非自動繼續：

### 必須暫停的情況

1. **方案選擇**：有多個可行方案，需用戶決定方向
2. **破壞性操作前**：刪除檔案、重置資料庫、force push 等不可逆操作
3. **超出原始範圍**：發現任務比預期複雜，需要擴大修改範圍
4. **依賴外部資源**：需要 API key、憑證、或用戶帳號資訊
5. **設計決策點**：架構選擇、技術選型等影響長期的決定
6. **調查結論**：完成調查後有多個可能的行動方案

### 暫停方式

將相關 todo 標記為 `pending`，在回覆中明確列出選項或問題，等待用戶回應。

不要因為收到 `TODO CONTINUATION` 就強行繼續，那只是提醒機制，最終決策權在你。
</todo_pause_conditions>
...
修改程式碼時，對於現有註解：
- **保留**：解釋 WHY 的註解、外部連結、決策理由、已知限制
- **評估精簡**：過於冗長或重複的註解，精簡但保留核心意圖
- **移除**：僅描述 WHAT 且程式碼本身已清楚表達的註解

**預設保守**：不確定時保留註解。移除有價值的註解比留下冗餘註解更糟。
